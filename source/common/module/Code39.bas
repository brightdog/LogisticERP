...
Sub Bar39(zObj As Object, zBarH As Integer, BarText As String, Optional zWithCheckSum As Boolean = False, Optional ByVal HasCaption As Boolean = False)
   ...
   End If
   'xObj.Height = (xObj.TextHeight(zBarText) + BarH + 25) * Screen.TwipsPerPixelY
   xObj.width = (((Len(zBarText) + IIf(WithCheckSum, 3, 2)) * 48 + 20)) * 16 * 2
...
Private Sub Draw_Bar(Encoding As String)
    ...
    intSpaceWidth = 3
    xObj.DrawWidth = 2
    xObj.DrawStyle = vbInsideSolid
    xObj.FillStyle = vbFSSolid
    For ii = 1 To Len(Encoding)
        xPos = xPos + 1
        xObj.Line (xPos + intSpaceWidth, xtop)-(xPos + intSpaceWidth, xtop + BarH), IIf(VBA.mID(Encoding, ii, 1), vbBlack, vbWhite)
    Next
    xPos = xPos + 5
    xObj.Line (xPos + intSpaceWidth, xtop)-(xPos + intSpaceWidth, xtop + BarH), vbWhite
End Sub
